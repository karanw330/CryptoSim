    ...
    if (isValid) {
        ...
        try {
            ...
            if (result.ok && result.data && result.data.username) {
                ...
                // Handle structured error vs simple string error from backend
                let errorMsg = 'Registration failed';
                if (result.data && result.data.detail) {
                    errorMsg = typeof result.data.detail === 'string' ? result.data.detail : (result.data.detail.content || errorMsg);
            ...
            showError('signupEmail', 'An unexpected error occurred');
            signupBtn.textContent = originalBtnText;
            signupBtn.disabled = false;
        }
    }
...
window.addEventListener('load', function () {
    ...
    // Hide OTP button from Signup form as requested
    const otpSignupBtn = document.getElementById('otpSignupBtn');
    if (otpSignupBtn) {
        otpSignupBtn.style.display = 'none';
    }
});
